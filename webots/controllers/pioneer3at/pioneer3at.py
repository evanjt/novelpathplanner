...
def cluster_points(3darray, ransac_threshold=0.98):
    # Create 2d array from the x and z columns (first and last)
    array = 3darray[:, [0,2]]
...
def capture_lidar_scene(lidar_device, path='points_evan.csv'):
    ...
    with open(path, 'w', newline='') as outfile:
        ...
        for i in range(10):
            ...
            for row in lidar_device.getPointCloud():
                ...
                # lidar_dist_threshold = 4
                # if row.y > -0.6 and \
                   # row.x == 0 and \
                   # row.y == 0 and \
                   # row.z == 0 and \
                   # row.x < lidar_dist_threshold and row.x > -lidar_dist_threshold and \
                   # row.z < lidar_dist_threshold and row.z > -lidar_dist_threshold:
                    csvwriter.writerow([row.x, row.y, row.z, row.layer_id])
